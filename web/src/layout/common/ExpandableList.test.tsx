...
describe('ExpandableList', () => {
  ...
  it('closes list when value when forceCollapseList is true', () => {
    const { getByTestId, rerender } = render(<ExpandableList items={getItems(12)} open />);

    const btn = getByTestId('expandableListBtn');
    expect(btn).toHaveTextContent('Show less...');

    rerender(<ExpandableList items={getItems(12)} open forceCollapseList />);
