...
const TransferChartRepositoryModal = (props: Props) => {
  ...
  async function transferChartRepository() {
    try {
      await API.transferChartRepository({
        chartRepositoryName: props.chartRepository.name,
      ...
      if (err.statusText !== 'ErrLoginRedirect') {
        let error = 'An error occurred transfering the chart repository';
        switch (err.status) {
          case 400:
  ...
  return (
    <Modal
      header={<div className={`h3 m-2 ${styles.title}`}>Transfer chart repository</div>}
      open={props.open}
        ...
        >
          {isSending ? (
            <>
              ...
              <span className="ml-2">Transfering chart repository...</span>
            </>
          ) : (
            <>Transfer</>
          )}
    ...
    >
      <div className="w-100">
        <form
          data-testid="transferChartRepoForm"
          ref={form}
...
export default TransferChartRepositoryModal;
