...
func TestGenerateVersions(t *testing.T) {
	...
	if *update {
		...
		for _, iv := range imgVersions {
			...
			err := os.WriteFile(name, iv.Data, 0600) // #nosec G703 -- test golden path uses versions generated by GenerateVersions
			require.NoError(t, err)
		}
	}
	for _, iv := range imgVersions {
		name := fmt.Sprintf("testdata/valid@%s.png", iv.Version)
		ivGolden, err := os.ReadFile(name) // #nosec G703 -- test golden path uses versions generated by GenerateVersions
		require.NoError(t, err)
