...
API calls will go to [http://localhost:8000](http://localhost:8000), so the `hub` server is expected to be up and running. *Please not that you'll also need to disable cross-origin restrictions in your browser.*
...
alias hub_server="pushd $HUB_SOURCE/cmd/hub; go run -race -mod=readonly *.go; popd"
alias hub_tracker="pushd $HUB_SOURCE/cmd/tracker; go run -race -mod=readonly main.go; popd"
